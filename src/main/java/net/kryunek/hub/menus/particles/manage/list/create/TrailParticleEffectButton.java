...
@AllArgsConstructor
public class TrailParticleEffectButton extends Button {
    ...
    public ItemStack getButtonItem(Player player) {
        ...
        return new ItemBuilder(iconMaterial)
                ...
                .lore(Arrays.asList(
                        CC.translate("&7Trail: &f" + (data == null ? "Unknown" : data.getTrailName())),
                        CC.translate("&7Particle: &f" + this.particle.name()),
                        CC.translate("&7Preview Icon: &f" + iconMaterial.name()),
                        "",
                        CC.translate("&bLeft click: &7Create with this particle")
                ))
                .build();
    }
    ...
    public void clicked(Player player, int slot, ClickType clickType, int hotbarButton) {
        ...
        if (data == null) {
            playFail(player);
            player.sendMessage(CC.translate("&cNo active trail creation session."));
            return;
        }

        boolean created = ModuleService.getManagerModule().getTrailParticleManager()
                .createTrail(data.getTrailName(), this.particle.name(), data.getMaterial(), data.getData());

        if (!created) {
            playFail(player);
            player.sendMessage(CC.translate("&cCould not create the trail particle."));
            return;
        }

        TrailParticleCreateSession.stop(player);
        playSuccess(player);
        player.sendMessage(CC.translate("&aTrail particle created: &f" + data.getTrailName()));
        new net.kryunek.hub.menus.particles.manage.list.TrailParticlePaginatedMenu().openMenu(player);
    }
